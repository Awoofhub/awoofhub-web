    ...
    <div className="bg-gray-50 pt-6 md:pt-12">
      ...
      >
        <div className=" max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12 flex flex-col md:flex-row md:gap-10 md:items-center md:justify-between">
          ...
          <ul
            ref={navRef}
            className="flex gap-4 flex-nowrap whitespace-nowrap no-scrollbar scroll-smooth overflow-x-auto"
          >
            {categories.map((category) => (
              <li
                key={category.id}
                ref={(el) => {
                  itemRefs.current[category.slug] = el;
                }}
                className={`px-5 py-1.5 font-semibold rounded-full text-sm whitespace-nowrap transition-all duration-200 ease-in-out ${
                  visibleSection === category.slug
